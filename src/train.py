...
from src.models.baselines import LogRegClassifier, XGBoostClassifier
...
def _train_pytorch(
    ...
    if type(data.edge_index).__name__ == "SparseTensor":
        row, col, _ = data.edge_index.coo()
        data.edge_index = torch.stack([row, col], dim=0)
    elif getattr(data.edge_index, "layout", None) == torch.sparse_coo:
        data.edge_index = data.edge_index.indices()
    ...
    for epoch in range(1, config.max_epochs + 1):
        ...
        logger.debug(
            json.dumps({
                "epoch": epoch,
                "train_loss": round(float(avg_train_loss), 5),
        ...
        if val_acc > best_val_acc:
            ...
            logger.info(
                ...
                epoch, val_acc, float(avg_train_loss),
            )
        else:
            epochs_no_improve += 1
...
def train_model(
    ...
    if isinstance(model, (LogRegClassifier, XGBoostClassifier)):
...
def _build_model_for_config(
    ...
    if model_cls is XGBoostClassifier:
...
def hyperparameter_search(
    ...
    log_path = results_dir / f"hparam_search_{base_config.encoder_name}_{base_config.model_name}.jsonl"
    
    logger.info("Optuna Hyperparameter search: model=%s encoder=%s n_trials=%d", 
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        for k, v in search_space.items():
            if isinstance(v, list) and isinstance(v[0], str):
                ptype = v[0]
                if ptype == "float":
                    is_log = (len(v) > 3 and v[3] == "log")
                    params[k] = trial.suggest_float(k, float(v[1]), float(v[2]), log=is_log)
                elif ptype == "int":
        ...
        trial_config = replace(
            ...
            xgb_n_estimators=int(params.get("xgb_n_estimators", base_config.xgb_n_estimators)),
            xgb_max_depth=int(params.get("xgb_max_depth", base_config.xgb_max_depth)),
            xgb_learning_rate=float(params.get("xgb_learning_rate", base_config.xgb_learning_rate)),
        )
    ...
    logger.info("Optuna search done. best_val=%.4f best_params=%s", best_val, best_params)
