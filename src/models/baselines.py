...
class XGBoostClassifier(nn.Module):
    """XGBoost classifier for tree-based baseline.
    
    Like LogReg, it trains via fit() rather than a PyTorch loop.
    """
    def __init__(
        self,
        in_channels: int,
    ...
    def fit(self, x: torch.Tensor, y: torch.Tensor) -> "XGBoostClassifier":
        X = x.detach().cpu().numpy()
        Y = y.detach().cpu().numpy()
        self._xgb.fit(X, Y)
        self._fitted = True
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        if not self._fitted:
            raise RuntimeError("XGBoostClassifier.forward() called before fit().")
        X = x.detach().cpu().numpy()
    ...
    def reset_parameters(self) -> None:
        import xgboost as xgb
        self._xgb = xgb.XGBClassifier(
            n_estimators=self._xgb.n_estimators,
            max_depth=self._xgb.max_depth,
            learning_rate=self._xgb.learning_rate,
            random_state=self._xgb.random_state,
            eval_metric="mlogloss",
            tree_method="hist", device="cuda"
        )
        self._fitted = False
